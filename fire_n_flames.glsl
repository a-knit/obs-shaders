...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    float shift = 1.327+sin(builtin_elapsed_time*2.0)/2.4;
    float alpha = 1.0;
    ...
    float dist = 3.5-sin(builtin_elapsed_time*0.4)/1.89;
    ...
    p.x -= builtin_elapsed_time/1.1;
    float q = fbm(p - builtin_elapsed_time * 0.01+1.0*sin(builtin_elapsed_time)/10.0);
    float qb = fbm(p - builtin_elapsed_time * 0.002+0.1*cos(builtin_elapsed_time)/5.0);
    float q2 = fbm(p - builtin_elapsed_time * 0.44 - 5.0*cos(builtin_elapsed_time)/7.0) - 6.0;
    float q3 = fbm(p - builtin_elapsed_time * 0.9 - 10.0*cos(builtin_elapsed_time)/30.0)-4.0;
    float q4 = fbm(p - builtin_elapsed_time * 2.0 - 20.0*sin(builtin_elapsed_time)/20.0)+2.0;
    q = (q + qb - .4 * q2 -2.0*q3  + .6*q4)/3.8;
    // vec2 r = vec2(fbm(p + q /2.0 + iTime * speed.x - p.x - p.y), fbm(p + q - iTime * speed.y));
    vec2 r = vec2(fbm(p + q /2.0 + builtin_elapsed_time * speed.x - p.x - p.y), fbm(p + q - builtin_elapsed_time * speed.y));
    vec3 c = mix(c1, c2, fbm(p + r)) + mix(c3, c4, r.x) - mix(c5, c6, r.y);
    // vec3 color = vec3(c * cos(shift * fragCoord.y / iResolution.y));
    vec3 color = vec3(c * cos(shift * fragCoord.y));
    color += .05;
    color.r *= .8;
    ...
    vec4 camColor = image.Sample(builtin_texture_sampler, fragCoord.xy);
