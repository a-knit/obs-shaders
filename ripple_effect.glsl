...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec2 uv = fragCoord.xy * builtin_uv_size / builtin_uv_size.y * 10. * exp2(-3./builtin_uv_size.x);
    vec2 p0 = floor(uv);
...
// #endif
            ...
            float t = fract(0.3*builtin_elapsed_time + hash12(hsh));
            vec2 v = p - uv;
    ...
    float intensity = mix(0.01, 0.15, smoothstep(0.1, 0.6, abs(fract(0.05*builtin_elapsed_time + 0.5)*2.-1.)));
    vec3 n = vec3(circles, sqrt(1. - dot(circles, circles)));
    //vec3 color = texture(iChannel0, uv/resolution - intensity*n.xy).rgb + 5.*pow(clamp(dot(n, normalize(vec3(1., 0.7, 0.5))), 0., 1.), 6.);
    vec3 color = image.Sample(builtin_texture_sampler, fragCoord - intensity*n.xy).rgb + 5.*pow(clamp(dot(n, normalize(vec3(1., 0.7, 0.5))), 0., 1.), 6.);
