...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    uv*=4.;
    // float t = iTime*.05;
    float t = builtin_elapsed_time*.05;
    uv*=rot2D(t);
    // vec3 snd = texture(iChannel1, UV).rgb;
    // vec3 vid = texture(iChannel0, UV-(snd.x*.05)).rgb;
    vec3 vid = image.Sample(builtin_texture_sampler, UV).rgb;
    uv = uv *vid.rb;
    //for(int i = 0; i< 1;i++) {
