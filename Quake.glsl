// original - https://www.shadertoy.com/view/XdcBzs
...
float pat(in vec2 p)
{
    float time = builtin_elapsed_time*0.75;
...
vec2 Shake(float maxshake, float mag)
{
    ...
    float time = builtin_elapsed_time*speed;			// speed of shake
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float mag = 0.5+sin(builtin_elapsed_time)*0.5;		// shake magnitude...
    ...
    uv *= 1.0-(maxshake*mag);
    vec3 col = image.Sample(builtin_texture_sampler, uv + shakexy).xyz;
